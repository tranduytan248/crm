...
$testDir = Join-Path ([IO.Path]::GetTempPath()) ('crm-workflow-tests-' + [Guid]::NewGuid().ToString('N'))
[IO.Directory]::CreateDirectory($testDir) | Out-Null
$testExe = Join-Path $testDir 'DigitalSalesWorkflowTests.exe'

$refs = @('Core.Cate.dll', 'Modules.Cate.dll', 'TSFramework.Libs.dll') |
    ForEach-Object { '/reference:' + (Join-Path $crmBin $_) }

Write-Host "Biên dịch bài test DigitalSalesWorkflowTests..." -ForegroundColor Cyan
& $csc /nologo /target:exe "/out:$testExe" /reference:System.dll /reference:System.Data.dll /reference:System.Xml.dll /reference:System.Core.dll $refs (Join-Path $PSScriptRoot 'DigitalSalesWorkflowTests.cs')
...
if ($exitCode -ne 0) {
    throw "Workflow tests failed with exit code $exitCode"
}
